...
APP=clipai
REPO="ryangerardwilson/clipai"
APP_HOME="$HOME/.${APP}"
INSTALL_DIR="$APP_HOME/bin"
APP_DIR="$APP_HOME/app"
CONFIG_PATH="${XDG_CONFIG_HOME:-$HOME/.config}/clipai/config.json"
...
# Ensure config file exists (XDG compliant)
ensure_config() {
  local cfg_path="$1"
  local cfg_dir
...
JSON
    chmod 600 "$cfg_path"
    print_message info "${MUTED}Created config at${NC} $cfg_path"
  fi
}
...
start_service() {
  if command -v systemctl >/dev/null 2>&1; then
    if systemctl --user daemon-reload >/dev/null 2>&1; then
      ...
      print_message info "${ORANGE}Could not daemon-reload user units. Start manually:${NC}"
      print_message info "  systemctl --user daemon-reload"
      print_message info "  systemctl --user enable --now $UNIT_NAME"
    fi
  else
    print_message info "${ORANGE}systemctl not found. Start manually if desired.${NC}"
  fi
}
...
if ! command -v wl-copy >/dev/null 2>&1 || ! command -v wl-paste >/dev/null 2>&1; then
  print_message info "${ORANGE}wl-clipboard not found. Install it for clipboard integration.${NC}"
fi

echo ""
print_message info "${MUTED}Installed ${NC}${APP}${MUTED} to ${NC}${INSTALL_DIR}/${APP}"
print_message info "${MUTED}Config:${NC} $CONFIG_PATH"
print_message info "${MUTED}Service:${NC} $UNIT_NAME (user)"
print_message info "${MUTED}Logs:${NC} journalctl --user -u $UNIT_NAME -f"
echo ""
